...
def run_bulk_validation(n=50): # Start with 50 to see results fast
    print(f"🚀 Starting Hybrid Bulk Validation (N={n})...")
    df = pd.read_csv("phishing_data.csv")
    sample_df = df.sample(n)
    
    # Must match the columns in your database.py
    ...
    print("-" * 30)
    print(f"📊 FINAL BULK ACCURACY: {(correct/n)*100:.2f}%")
    print("-" * 30)

if __name__ == "__main__":
    run_bulk_validation(200)